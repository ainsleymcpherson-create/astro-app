...
if "auth" not in st.secrets or not st.user.is_logged_in:
    ...
    with st.expander("Add another login email"):
        ...
        with st.form("add_email_form"):
            ...
            _confirm_email_input = st.text_input("Confirm email address", placeholder="you@example.com")
            if st.form_submit_button("Send confirmation email"):
                _new_clean = _new_email_input.strip().lower()
                _confirm_clean = _confirm_email_input.strip().lower()
                if not _new_clean:
                    st.error("Enter an email address first.")
                elif not _confirm_clean:
                    st.error("Retype the email address in the confirm field.")
                elif _new_clean != _confirm_clean:
                    st.error("Those two email addresses don't match — check for typos.")
                else:
                    _ok, _result = request_email_change(user_email, _new_clean)
                    if not _ok:
                        st.error(_result)
                    else:
