...
if _unlock_claim and _unlock_claim.get("reading_type") in ("General", "Career / Work"):
    ...
    with _unlock_claim_slot.container():
        ...
        _uc_reading_type = _unlock_claim.get("reading_type", "General")
        st.success(
        ...
        with st.spinner("Setting up your chart..."):
            ...
            _uc_today = date_type.today()
            _uc_birth_date_obj = _uc_bd.date()
            _uc_age = (
                _uc_today.year - _uc_birth_date_obj.year
                - ((_uc_today.month, _uc_today.day) < (_uc_birth_date_obj.month, _uc_birth_date_obj.day))
            )
            if _uc_reading_type == "Career / Work":
                if _uc_unknown_time:
                    _uc_prompt = build_career_interpretation_prompt_no_time(
                        _uc_chart, _uc_aspects, _uc_patterns, _uc_dignities, person_name=_unlock_claim.get("label"),
                    )
                else:
                    _uc_prompt = build_career_interpretation_prompt(
                        _uc_chart, _uc_aspects, _uc_patterns, _uc_dignities, _uc_house_readings,
                        person_name=_unlock_claim.get("label"),
                    )
            else:
                if _uc_unknown_time:
                    _uc_prompt = build_interpretation_prompt_no_time(
                        _uc_chart, _uc_aspects, _uc_patterns, _uc_dignities,
                        person_name=_unlock_claim.get("label"), age=_uc_age,
                    )
                else:
                    _uc_prompt = build_interpretation_prompt(
                        _uc_chart, _uc_aspects, _uc_patterns, _uc_dignities, _uc_house_readings,
                        person_name=_unlock_claim.get("label"), age=_uc_age,
                    )
...
if _unlock_result and _unlock_result.get("reading_type") in ("General", "Career / Work"):
    with _unlock_claim_slot.container():
        _ur_label = _unlock_result.get("label") or "Your"
        if _unlock_result.get("text"):
            _ur_col1, _ur_col2 = st.columns([5, 1])
            with _ur_col1:
                st.subheader(f"{_ur_label}'s Full {_unlock_result.get('reading_type', 'General')} Reading")
            with _ur_col2:
            ...
            _ur_pdf_title = (
                f"{_ur_label}'s {_unlock_result.get('reading_type', 'General')} Reading"
                if _unlock_result.get("label") else
                f"{_unlock_result.get('reading_type', 'General')} Reading"
            )
