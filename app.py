...
"""
Runtime retrieval module. Import this into app.py.

Loads the precomputed embeddings JSON once (cache it in Streamlit),
embeds the incoming query, and returns the top-k most similar chunks
via plain numpy cosine similarity — no vector DB needed at this scale.

Supports optional filtering by category (e.g. "personal_readings" vs
"synastry") so a natal reading query doesn't pull in synastry-specific
...
def retrieve(
    ...
    """
    If category is given (e.g. "personal_readings" or "synastry"), only
    chunks tagged with that category are considered. Pass None to search
    across all categories.
    """
    if category is not None:
        indices = [i for i, c in enumerate(chunks) if c.get("category") == category]
        if not indices:
            return []
    ...
    similarities = sub_matrix @ query_vec  # cosine similarity, both normalized
