...
First, provide a SHORT overview of the chart — an orientation before \
the detailed themes. This is the fast, in-app version, so keep it \
tight: exactly 2 SEPARATE paragraphs with a blank line between them \
(not one continuous block, and not chunked or bulleted). Head this \
section with the exact markdown heading "## Overview" (two hash \
symbols, one space, then the word). The Overview uses the SAME naming \
convention as the "What This Means" sections described below — name \
planets, signs, houses, angles, lesser-used points, and aspect words \
directly, each glossed in plain English, e.g. "your drive (Mars)." \
GLOSS EVERY SIGN THE FIRST TIME IT'S NAMED IN THE OVERVIEW too, \
briefly: "Your core identity (the Sun) sits in Capricorn, disciplined \
and ambitious." Structure the two paragraphs like this:
- FIRST PARAGRAPH: OPEN WITH A PUNCHY DECLARATIVE THESIS, BUT ONE \
ACTUALLY GROUNDED IN THIS SPECIFIC CHART — one or two short, confident \
sentences, with no hedging and no astrology in them at all. The \
sentence must be something that would be FALSE or at least not \
obviously true of a random other person's chart — a real claim this \
specific data supports, not a general truth about human psychology \
that could open literally anyone's reading. "You don't experience \
yourself in isolation" is NOT acceptable: it's true of everyone. \
"Your career is in a state of ambitious reconstruction" IS acceptable, \
because it commits to a specific claim. Then, in the SAME paragraph, \
briefly name the ONE or TWO most notable clusters or groupings that \
support that thesis — if three or more points sit together in one \
sign and house (a stellium), or several planets are conjunct, name \
which planets (each glossed), which sign, which house, and what that \
fusion produces, weaving in dignity as causal language where relevant \
(e.g. a planet in Rulership "operates at full strength in its own \
sign").
- SECOND PARAGRAPH: step back and name the single most important \
pattern that recurs across MULTIPLE different placements or life \
areas — if hard aspects (squares, oppositions) repeat across several \
unrelated points, say so explicitly as a defining structural feature \
of the whole chart; do the same for repeated ease (trines, sextiles) \
if that's the real pattern instead. End with a short synthesizing \
...
Then, identify exactly 2 THEMES — the two biggest ones that emerge \
when you look at the whole chart together — which placements \
reinforce each other, which create tension, and why. This is the \
fast, in-app version, so keep it to exactly 2, not more. Format each \
theme's heading as a markdown H2 heading — exactly "## Theme Name" \
(two hash symbols, one space, then the name) — since the app \
displaying this reading relies on that exact format to build a \
collapsible view. Then follow the three-part format described below \
for each one.
...
lead-in. THEN follow with a three-part structure, IN THIS ORDER:
    **Summary:** Written FIRST, immediately after the 1-2 sentence \
    ...
    actual point. No jargon, no chunking, just the plain-language core \
    of what this theme means for the person's life.
    **What This Means:** Written SECOND. Break it into 2-3 short, \
    scannable chunks, each starting with a bolded claim stated as a \
    short phrase followed by a colon (e.g. "**Speaking and thinking are \
    central to your identity:**"). Within each chunk, you MAY name any \
    point directly — planets, signs, angles (Midheaven, Ascendant, \
...
def build_interpretation_prompt_fast(
    chart: dict[str, ChartPoint],
    aspects: list[Aspect],
    patterns: dict[str, list[AspectPattern]],
    dignities: dict[str, DignityResult],
    ...
    """
    Condensed variant of build_interpretation_prompt for the LIVE,
    in-app "Generate Full Reading" path specifically -- exactly 2
    themes and a 2-paragraph Overview instead of 2-4/4-6, since this
    path has someone actively waiting on the page. The emailed full
    reading (via email_worker) is unaffected and still uses the full
    build_interpretation_prompt -- length isn't a constraint there
    since nobody's watching a spinner for it.
    """
    data_block = build_data_block(
        chart, aspects, patterns, dignities, house_readings,
        min_tightness=min_tightness,
    ...
    query = _build_retrieval_query(chart, aspects, dignities)
    reference_block = _reference_context_block(query, category="personal_readings")
    return INTERPRETATION_INSTRUCTIONS_FAST.format(
        data_block=data_block,
        naming_note=_single_person_naming_note(person_name),
        age_guidance=_age_guidance(age),
        reference_block=reference_block,
