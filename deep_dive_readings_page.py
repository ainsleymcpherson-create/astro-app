...
if _unlock_claim and _unlock_claim.get("reading_type") in ("Lilith", "Chiron", "North/South Node"):
    ...
    with _unlock_claim_slot.container():
        _uc_label = _unlock_claim.get("label") or "your"
        _uc_reading_type = _unlock_claim.get("reading_type", "Lilith")
        st.success(
            f"🎉 Payment confirmed for {_uc_label} {_uc_reading_type} reading "
            f"— generating it now. This can take a few minutes; keep this "
            f"tab open until it finishes.",
            icon="🎉",
        )
        with st.spinner("Setting up your chart..."):
            ...
            # Deep dive readings don't have a no-time prompt variant
            # (unlike General/Career) -- the normal builder is always
            # used regardless of _uc_unknown_time, matching this page's
            # own existing dispatch above.
            if _uc_reading_type == "Chiron":
                _uc_prompt = build_chiron_deep_dive_prompt(
                    _uc_chart, _uc_aspects, _uc_patterns, _uc_dignities, _uc_house_readings,
                    person_name=_unlock_claim.get("label"),
                )
            elif _uc_reading_type == "North/South Node":
                _uc_prompt = build_lunar_nodes_deep_dive_prompt(
                    _uc_chart, _uc_aspects, _uc_patterns, _uc_dignities, _uc_house_readings,
                    person_name=_unlock_claim.get("label"),
                )
            else:
                _uc_prompt = build_lilith_deep_dive_prompt(
                    _uc_chart, _uc_aspects, _uc_patterns, _uc_dignities, _uc_house_readings,
                    person_name=_unlock_claim.get("label"),
                )
        ...
        st.session_state["_unlock_result"] = {
            ...
            "label": _unlock_claim.get("label"), "reading_type": _uc_reading_type,
        }

_unlock_result = st.session_state.get("_unlock_result")
if _unlock_result and _unlock_result.get("reading_type") in ("Lilith", "Chiron", "North/South Node"):
    with _unlock_claim_slot.container():
        _ur_label = _unlock_result.get("label") or "Your"
        if _unlock_result.get("text"):
            _ur_col1, _ur_col2 = st.columns([5, 1])
            with _ur_col1:
                st.subheader(f"{_ur_label}'s Full {_unlock_result.get('reading_type', 'Lilith')} Reading")
            with _ur_col2:
            ...
            _ur_pdf_title = (
                f"{_ur_label}'s {_unlock_result.get('reading_type', 'Lilith')} Reading"
                if _unlock_result.get("label") else
                f"{_unlock_result.get('reading_type', 'Lilith')} Reading"
            )
        ...
        if r["interpretation_text"]:
            ...
            if r.get("want_full_now"):
                ...
                summary_pdf_bytes = _get_or_build_pdf(
                    "deep_dive_reading_summary_pdf", summary_text, f"{pdf_title} \u2014 Summary", pdf_subtitle,
                )
                full_pdf_bytes = _get_or_build_pdf(
                    "deep_dive_reading_full_pdf", full_text, pdf_title, pdf_subtitle,
                )
                ...
                pdf_bytes = _get_or_build_pdf(
                    "deep_dive_reading_pdf", r["interpretation_text"], pdf_title, pdf_subtitle,
                )
